...
const QuestionScreen = ({ navigation }) => {
    ...
    const finalResult = () => {
        ...
        if(correctAnswersNumber < 20)
            finalQuote = ' Creo que prefieres Harry Potter...';
        else if(correctAnswersNumber < 40)
            finalQuote = ' No eres un gran fan...'
        else if(correctAnswersNumber < 60)
            finalQuote = ' Bueno... no está mal';
        else if(correctAnswersNumber < 80)
            finalQuote = ' Muy bien! sabes más de lo que creía';
        else if(correctAnswersNumber < 99)
        ...
        Alert.alert('Has llegado hasta el final', `Has conseguido una puntuacion de ${correctAnswersNumber} sobre 100.${finalQuote}`, [
                {text: 'Aceptar', onPress: () => navigation.navigate('¡Hola!') },
                {text: 'Presumir', onPress: () => {onShare()}  },
            ]);
    }
    ...
    const onShare = async () => {
        try {
          const content = {
            message:`${correctAnswersNumber} de 100. Esa es mi puntuación en El Señor de los Anillos Trivia. Te reto a superarme! \nhttps://play.google.com/store/apps/details?id=com.JarlGab.JungleGame`,
            title: '¿Puedes superarme?'
          }
          const options = {
            dialogTitle: 'Compártelo con todo el mundo!',
          }
